...
export default function Chatbot() {
  ...
  const [pdflink, setPfdflink] = useState("")
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      console.log("API full response:", data); // Log the entire response to inspect it
  
      if (response.ok) {
        const pdfUrl = data.data[0]; // Assuming first item is the PDF URL
        if (pdfUrl) {
    ...
    <div className="chat-bg p-4">
      <div style={{ maxWidth: "50%", margin: "0 auto" }}
        ...
        <div ref={chatContainerRef}
          style={{ height: "75vh", overflowY: "auto", padding: "1rem", marginBottom: "1rem" }}>
          {chatHistory.map((chat, index) => (
            <div key={index}
              style={{ display: "flex", justifyContent: chat.sender === "user" ? "flex-end" : "flex-start", marginBottom: "1rem" }}>
              <div style={{ color: chat.sender === "user" ? "#fff" : "#fff", maxWidth: "75%" }}
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", gap: "1rem" }}
          ...
          <button
            type="submit"
            style={{ padding: "0.5rem 1rem", backgroundColor: "#7C72FF", color: "#fff", border: "none", borderRadius: "5px" }}>
        ...
        <button
          ...
          style={{ padding: "0.5rem 1rem", backgroundColor: "#28a745", color: "#fff", border: "none", borderRadius: "5px", marginTop: "1rem" }}>
          Generate PDF
        </button>
